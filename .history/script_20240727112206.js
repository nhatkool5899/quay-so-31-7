...
async function loadExcel_spin() {
    await fetch('data-code-spin.xlsx')
        .then(response => response.arrayBuffer())
        .then(d => {
            const workbook = XLSX.read(d, { type: 'array' });
            const firstSheet = workbook.Sheets[workbook.SheetNames[0]];
...
// window.onload = (loadExcel, loadExcel_spin);

// window.onload = loadExcel_spin;

async function startRandom() {

    audioElement.play();

    if (data.length === 0 || data_spin === 0) {
    ...
    intervalId = setInterval(() => {
        ...
        const winners = getRandomEntries(data_spin, quantity[number_arr]);
